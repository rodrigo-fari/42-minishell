...
/*   Updated: 2024/12/07 21:35:43 by rde-fari         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 * 
 * This function appends a new `t_env` node to the end of an existing linked list. 
 * If the list is empty (i.e., the head is `NULL`), the new node becomes the head.
 * Otherwise, the function finds the last node and links it to the new node.
 * 
 ...
 * 
 * @note 
 * - The function assumes that the `new` node is properly allocated and initialized.
 ...
 * 
 * If `new` is a node with key = "HOME" and value = "/home/user", after calling 
 * `listadd_back`, the list will become:
 * - Node 1: key = "PATH", value = "/usr/bin"
 ...
 * 
 * This function iterates through the linked list of `t_env` nodes until it reaches 
 * the last node (i.e., the node whose `next` pointer is `NULL`). It returns a pointer 
 * to this last node. If the list is empty, it returns `NULL`.
 * 
